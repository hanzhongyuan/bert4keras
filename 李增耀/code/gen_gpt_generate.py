...
config_path = 'E:/bert_model/model/chinese_nezha_gpt_L-12_H-768_A-12/config.json'
checkpoint_path = 'E:/bert_model/model/chinese_nezha_gpt_L-12_H-768_A-12/gpt.ckpt'
dict_path = 'E:/bert_model/model/chinese_nezha_gpt_L-12_H-768_A-12/vocab.txt'
...
class ArticleCompletion(AutoRegressiveDecoder):
    ...
    def generate(self, text, topk=10):
        token_ids = tokenizer.encode(text)[0][:-1]
        # print(token_ids)
        # results = self.random_sample([token_ids], n, topp=topp)  # 基于随机采样

        results = self.beam_search([token_ids], topk=topk)  # 基于beam_search采样
        # print(results)
        # print(ids for ids in results)

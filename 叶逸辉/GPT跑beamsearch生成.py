...
config_path = 'F:/0：学习资料/0：研究生/NLP/123/chinese_nezha_gpt_L-12_H-768_A-12/config.json'
checkpoint_path = 'F:/0：学习资料/0：研究生/NLP/123/chinese_nezha_gpt_L-12_H-768_A-12/gpt.ckpt'
dict_path = 'F:/0：学习资料/0：研究生/NLP/123/chinese_nezha_gpt_L-12_H-768_A-12/vocab.txt'
...
class ArticleCompletion(AutoRegressiveDecoder):
    ...
    def generate(self, text, n=9, topp=0.95):
        token_ids = tokenizer.encode(text)[0][:-1]
        results = self.beam_search([token_ids], n)
        print([text + tokenizer.decode(results) ])
...
#
for i in range(5,10):
    article_completion.generate(u'抄袭检测',n=i)
